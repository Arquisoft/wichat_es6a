...
const historyServiceUrl =
  process.env.HISTORY_SERVICE_URL || "http://localhost:8010";
const questionServiceUrl =
  process.env.QUESTION_SERVICE_URL || "http://localhost:8005";
const wikidataServiceUrl =
  process.env.WIKIDATA_SERVICE_URL || "http://localhost:8020";
...
app.get("/user/:id", async (req, res) => {
  try {
    const userResponse = await axios.get(
      userServiceUrl + `/user/${req.params.id}`
    );
...
app.put("/user/:id/username", async (req, res) => {
  try {
    const userResponse = await axios.put(
      userServiceUrl + `/user/${req.params.id}/username`,
      req.body
    );
    res.json(userResponse.data);
  } catch (error) {
...
app.put("/user/:id/password", async (req, res) => {
  try {
    const userResponse = await axios.put(
      userServiceUrl + `/user/${req.params.id}/password`,
      req.body
    );
    res.json(userResponse.data);
  } catch (error) {
...
app.get("/user/:id/profile-pic", async (req, res) => {
  try {
    const userResponse = await axios.get(
      userServiceUrl + `/user/${req.params.id}/profile-pic`,
      { responseType: "stream" }
    );
...
app.post("/generateQuestions", async (req, res) => {
  try {
    const llmResponse = await axios.post(
      llmServiceUrl + "/generateQuestions",
      req.body
    );
...
app.post("/configureAssistant", async (req, res) => {
  try {
    const llmResponse = await axios.post(
      llmServiceUrl + "/configureAssistant",
      req.body
    );
    res.json(llmResponse.data);
  } catch (error) {
...
app.post("/getHintWithQuery", async (req, res) => {
  try {
    const llmResponse = await axios.post(
      llmServiceUrl + "/getHintWithQuery",
      req.body
    );
    res.json(llmResponse.data);
  } catch (error) {
...
app.get("/api/entries/random", async (req, res) => {
  try {
    const wikiResponse = await axios.get(
      wikidataServiceUrl + "/api/entries/random"
    );
    res.json(wikiResponse.data);
  } catch (error) {
    failedRequestsCounter.inc({
      service: "wikidata",
      endpoint: "/api/entries/random",
      status: error.response?.status || 500,
    });
    res
      .status(error.response?.status || 500)
...
app.get("/api/entries/:category", async (req, res) => {
  try {
    const wikiResponse = await axios.get(
      wikidataServiceUrl + `/api/entries/${req.params.category}`,
      {
        params: req.query,
      }
    );
...
app.post("/api/entries/fetch/:category", async (req, res) => {
  try {
    const wikiResponse = await axios.post(
      wikidataServiceUrl + `/api/entries/fetch/${req.params.category}`,
      req.body,
      {
        params: req.query,
      }
    ...
    failedRequestsCounter.inc({
      service: "wikidata",
      endpoint: "/api/entries/fetch/:category",
      status: error.response?.status || 500,
    });
    res
      .status(error.response?.status || 500)
...
app.get("/api/monumentos", async (req, res) => {
  try {
    const wikiResponse = await axios.get(
      wikidataServiceUrl + "/api/monumentos"
    );
    res.json(wikiResponse.data);
  } catch (error) {
    failedRequestsCounter.inc({
      service: "wikidata",
      endpoint: "/api/monumentos",
      status: error.response?.status || 500,
    });
    res
      .status(error.response?.status || 500)
