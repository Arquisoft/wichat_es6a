...
describe('HowToPlayWindow', () => {
  ...
  it('renders the header section with correct text', () => {
    ...
    expect(headerSection).toHaveStyle({ textAlign: 'center', marginTop: '32px', marginBottom: '48px' });
  ...
  it('renders the Introduction section with correct text and image', () => {
    ...
    expect(introductionCard).toHaveStyle({
      background: 'linear-gradient(135deg, #e0f2ff, #c2e5ff, #a8d8ff)',
      borderRadius: '16px',
    });

    expect(screen.getByTestId('introduction-title')).toHaveTextContent('Introducción');
    ...
    texts.forEach(({ testId, content }) => {
      ...
      expect(textElement).toHaveStyle({ textAlign: 'justify', color: '#000000' });
    });

    const image = screen.getByTestId('introduction-image');
    expect(image).toHaveAttribute('src', './info-window/info-introducction.png');
  ...
  it('renders the Wildcards section with correct text and images', () => {
    ...
    expect(wildcardsCard).toHaveStyle({
      background: 'linear-gradient(135deg, #e0f2ff, #c2e5ff, #a8d8ff)',
      borderRadius: '16px',
    });

    expect(screen.getByTestId('wildcards-title')).toHaveTextContent('Comodines');
    ...
    const images = [
      { testId: 'wildcard-pista-image', src: './info-window/comodin-pista.png', alt: 'Captura del comodín pista', width: '35%' },
      {
      ...
      { testId: 'wildcard-50-50-image', src: './info-window/comodin-50-50.png', alt: 'Captura del comodín 50/50', width: '35%' },
    ];
  ...
  it('renders the Game Modes section with correct text and image', () => {
    ...
    expect(gameModesCard).toHaveStyle({
      background: 'linear-gradient(135deg, #e0f2ff, #c2e5ff, #a8d8ff)',
      borderRadius: '16px',
    });

    expect(screen.getByTestId('game-modes-title')).toHaveTextContent('Modos de juego');
  ...
  it('renders the Difficulties section with correct text and images', () => {
    ...
    expect(difficultiesCard).toHaveStyle({
      background: 'linear-gradient(135deg, #e0f2ff, #c2e5ff, #a8d8ff)',
      borderRadius: '16px',
    });

    expect(screen.getByTestId('difficulties-title')).toHaveTextContent('Dificultades');
  ...
  it('renders the Scoring section with correct text and images', () => {
    ...
    expect(scoringCard).toHaveStyle({
      background: 'linear-gradient(135deg, #e0f2ff, #c2e5ff, #a8d8ff)',
      borderRadius: '16px',
    });
